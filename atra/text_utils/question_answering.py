...
def answer_question(text, question, input_lang) -> str:
    text = f"""Answer the given Question with the following Context: {text}
    Question: {question}"""
...
def inference_qa(model, input_features):
    ...
    with torch.inference_mode():
        generated_tokens = model.generate(inputs=input_features, max_new_tokens=1024, do_sample=False, num_beams=5, early_stopping=True)
